...
(function($){
	...
	function Page(container,fn,args,style){
		...
		//建立自己的容器
		this.container.html('<div class="pagination_inside"></div>');
	...
	Page.prototype.fillHtml=function(){
		...
		if(args.showPrev){
			if(args.current > 1){
				obj.append('<div class="icon_set icon_set_pre "><a href="javascript:;" class="icon icon_pre prevPage"></a></div>');
			}else{
				obj.remove('.prevPage');
				obj.append('<div class="icon_set icon_set_pre "><span class="icon icon_pre disabled"></span></div>');
			}
		}
		//中间页码
		if(args.current != 1){
		...
		if(args.showNext){
			if(args.current== args.pageCount||args.pageCount==1){
				obj.remove('.nextPage');
				obj.append('<div class="icon_set icon_set_next"><span class="icon icon_next disabled"></span></div>');
			}else{
				obj.append('<div class="icon_set icon_set_next"><a href="javascript:;" class="icon icon_next nextPage"></a></div>');
			}
		}
