...
class ECPayController extends AbstractController
{
     ...
     * @param $userOrderRepository
     * @return Response
     * @throws \SendGrid\Mail\TypeException
     */
    function pay(ProductRepository $productRepository, UserOrderRepository $userOrderRepository)
    {
        ...
        if($cart->getCouponMessage())
            $order->setCouponMessage($cart->getCouponMessage());
        else
            $order->setCouponMessage('');
         ...
         */
        $email= new Mail();
        $email->setFrom("no-reply@littlefarmer.com", "格雷朵");
        ...
        {
            //計算ecpay 需要的資料
            $name = $name.$relation->getProduct()->getZhName()."x".$relation->getQuantity()."#";
        }
...
//        $ecpay->CheckMacValue = ECPay_CheckMacValue::generate( $arParameters, $farmer->getHashKey(), $farmer->getHashIV() );
    ...
    public function ECPayCompleted(UserOrderRepository $userOrderRepository)
    {
        ...
        foreach ($relations as $relation)
        {
            ...
            $productToUserOrder->setSize($relation->getSize());
            $productToUserOrder->setPrice($relation->getPrice());
            $em->persist($productToUserOrder);

            //處理庫存
            $stockArray = $product->getStock();
            $stock = $product->getStock()[$index];
            $stockArray[$index] = $stock - $relation->getQuantity();
